...
<div class="document-form">
    ...
    <?php
    // Admin sin record_id definido: mostrar selector de expediente
    if ($isAdmin && !$record):
        $allRecords = ArrayHelper::map(
            Record::find()->with('teaching')->all(),
            'id',
            function($r) {
                $nombre = $r->teaching->nombreCompleto ?? 'Sin datos';
                return "Exp. #{$r->id} — {$nombre}";
            }
        );
    ?>
    ...
    <?= $form->field($model, 'archivo')->fileInput([
        ...
        'class'  => 'form-control',
    ])->label('Archivo (PDF, Word, imagen — máx. 10MB)') ?>
